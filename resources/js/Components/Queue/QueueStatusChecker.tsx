...
export default function QueueStatusChecker({ onQueueFound }: Props) {
    ...
    const getStatusIcon = (status: string) => {
        switch (status) {
            ...
            case 'called': return <Ticket className="w-4 h-4" />;
    ...
    const getStatusMessage = (status: string) => {
        switch (status) {
            ...
            case 'called': return 'Your number has been called! Please proceed to the counter.';
            case 'serving': return 'You are currently being served';
            case 'completed': return 'Your service has been completed';
            case 'cancelled': return 'Your queue number has been cancelled';
                ...
                {foundQueue && (
                    <div className="mt-6 p-4 bg-muted rounded-lg">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <Badge className={getStatusColor(foundQueue.status)}>
                                <div className="flex items-center gap-1">
                                    {getStatusIcon(foundQueue.status)}
                                    {foundQueue.status.charAt(0).toUpperCase() + foundQueue.status.slice(1)}
                                </div>
                            </Badge>
                        </div>
                        ...
                        <div className="space-y-2 text-sm">
                            <div>
                                <span className="font-medium">Service:</span> {foundQueue.queue_type.name}
                            </div>
                            {foundQueue.customer_name && (
                                <div>
                                    <span className="font-medium">Name:</span> {foundQueue.customer_name}
                                </div>
                            )}
                            <div>
                                <span className="font-medium">Status:</span> {getStatusMessage(foundQueue.status)}
                            </div>
                            {foundQueue.called_at && (
                                <div>
                                    <span className="font-medium">Called at:</span> {new Date(foundQueue.called_at).toLocaleTimeString()}
                                </div>
                            )}
                        </div>
                    </div>
