...
export default function ParticipantTimer({ 
    ...
    const fetchTimerStatus = useCallback(async () => {
        try {
            ...
            console.log('Fetching timer status for participant:', participantId);
            const response = await axios.get(`/challenges/${challengeId}/timer/${participantId}/status`);
            ...
            if (status.formatted_time) {
                console.log('Using backend formatted time:', status.formatted_time);
                ...
                const minutes = Math.floor((status.elapsed_time_seconds % 3600) / 60);
                const seconds = status.elapsed_time_seconds % 60;
                const fallbackTime = formatTime(hours, minutes, seconds);
                console.log('Using fallback formatted time:', fallbackTime, 'from elapsed seconds:', status.elapsed_time_seconds);
                setDisplayTime(fallbackTime);
            }
            
            // Start interval if timer is running
            if (status.is_running) {
                console.log('Timer is running, starting display timer with elapsed seconds:', status.elapsed_time_seconds);
    ...
    const startDisplayTimer = useCallback((initialSeconds: number) => {
        ...
        setDisplayTime(formatTime(initialHours, initialMinutes, initialSecondsRemaining));
    ...
    const startTimer = async () => {
        setIsLoading(true);
        try {
            const response = await axios.post(`/challenges/${challengeId}/timer/start`, {
                participant_id: participantId
            });
            
            // Get the updated timer status after starting
            const statusResponse = await axios.get(`/challenges/${challengeId}/timer/${participantId}/status`);
    ...
    const stopTimer = async () => {
        setIsLoading(true);
        try {
            const response = await axios.post(`/challenges/${challengeId}/timer/stop`, {
                participant_id: participantId
            });
    ...
    const pauseTimer = async () => {
        setIsLoading(true);
        try {
            const response = await axios.post(`/challenges/${challengeId}/timer/pause`, {
                participant_id: participantId
            });
    ...
    const resumeTimer = async () => {
        setIsLoading(true);
        try {
            const response = await axios.post(`/challenges/${challengeId}/timer/resume`, {
                participant_id: participantId
            });
    ...
    const resetTimer = async () => {
        setIsLoading(true);
        try {
            const response = await axios.post(`/challenges/${challengeId}/timer/reset`, {
                participant_id: participantId
            });
            ...
            <CardContent>
                <div className="text-center mb-6">
                    ...
                    <div className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                        {isInitialLoading ? 'Loading timer status...' :
                         timerStatus?.is_running ? 'Running' : 
                         timerStatus?.has_started ? 'Paused' : 'Not Started'}
                    </div>
                </div>
                ...
                {timerStatus?.has_started && (
                    <div className="mt-4 text-xs text-gray-500 dark:text-gray-400 text-center">
                        {timerStatus.timer_started_at && (
                            <div>Started: {new Date(timerStatus.timer_started_at).toLocaleString()}</div>
                        )}
                        {timerStatus.timer_ended_at && (
                            <div>Ended: {new Date(timerStatus.timer_ended_at).toLocaleString()}</div>
                        )}
                    </div>
                )}
            </CardContent>
