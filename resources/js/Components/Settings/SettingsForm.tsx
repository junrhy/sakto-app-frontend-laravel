...
export default function SettingsForm({
    ...
    const form = useForm({
        general: {
            clinic_name: settings.general?.clinic_name || '',
        ...
        appointments: {
            enable_appointments: settings.appointments?.enable_appointments ?? true,
            appointment_duration: settings.appointments?.appointment_duration ?? 30,
            ...
            enable_online_booking: settings.appointments?.enable_online_booking ?? true
        },
        features: {
            enable_insurance: settings.features?.enable_insurance ?? true,
            insurance_providers: settings.features?.insurance_providers || [],
            enable_prescriptions: settings.features?.enable_prescriptions ?? true,
            enable_lab_results: settings.features?.enable_lab_results ?? true,
            enable_dental_charts: settings.features?.enable_dental_charts ?? true,
            enable_medical_history: settings.features?.enable_medical_history ?? true,
            enable_patient_portal: settings.features?.enable_patient_portal ?? true
        },
        billing: {
            ...
            invoice_footer: settings.billing?.invoice_footer || 'Thank you for choosing our clinic!'
        }
    });
