...
export default function CircularView({ familyMembers }: FamilyTreeProps) {
    ...
    const [selectedRoot, setSelectedRoot] = useState<number | string | undefined>(undefined);
    const [navigationHistory, setNavigationHistory] = useState<(number | string)[]>([]);
    ...
    const handleBack = () => {
        if (navigationHistory.length > 0) {
            ...
            const previousRoot = navigationHistory[navigationHistory.length - 1];
