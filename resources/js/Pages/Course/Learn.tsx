...
import { ArrowLeft, Play, Clock, CheckCircle, Circle, BookOpen, Video, FileText, Award, User, Download } from 'lucide-react';
...
export default function Learn({ auth, course, progress, flash, viewingContact }: Props) {
    const [lessons, setLessons] = useState<Lesson[]>([]);
    const [currentLesson, setCurrentLesson] = useState<Lesson | null>(null);
    const [loading, setLoading] = useState(true);
    const [currentProgress, setCurrentProgress] = useState<Progress | null>(progress || null);
    ...
    const markLessonAsStarted = async (lessonId: number) => {
        if (!isEnrolled) {
            toast.error('Contact is not enrolled in this course. Please enroll first.');
            return;
        }
        ...
        try {
            const response = await axios.post(`/course-enrollments/${currentProgress.enrollment_id}/progress/${lessonId}/start`);
            ...
            if (currentLesson?.id === lessonId) {
                setCurrentLesson(prev => prev ? { ...prev, status: 'in_progress' as const } : null);
            }
        } catch (error) {
    ...
    const markLessonAsCompleted = async (lessonId: number) => {
        ...
        try {
            const response = await axios.post(`/course-enrollments/${currentProgress.enrollment_id}/progress/${lessonId}/complete`);
            ...
            if (currentLesson?.id === lessonId) {
                setCurrentLesson(prev => prev ? { ...prev, status: 'completed' as const } : null);
            }
            ...
            const completedLessons = lessons.filter(l => l.status === 'completed').length + 1;
            const newProgressPercentage = Math.round((completedLessons / lessons.length) * 100);
            
            if (newProgressPercentage >= 100) {
                toast.success('Congratulations! You have completed the course. You can now download your certificate.');
            }
        } catch (error) {
            ...
            if (data.data && typeof data.data.is_enrolled === 'boolean') {
                ...
                if (data.data.is_enrolled && data.data.enrollment) {
                    setCurrentProgress({
                        ...
                        course_id: course.id,
                        progress_percentage: data.data.enrollment.progress_percentage || 0,
                        lessons_completed: data.data.enrollment.lessons_completed || 0,
    ...
    const handleDownloadCertificate = async () => {
        ...
        try {
            ...
            let response = await axios.get(`/course-enrollments/${currentProgress.enrollment_id}/certificate/get`);
            
            // If certificate doesn't exist, generate it
            if (!response.data.success && response.status === 400) {
                toast.loading('Generating certificate...');
                response = await axios.get(`/course-enrollments/${currentProgress.enrollment_id}/certificate`);
            }
    ...
    if (loading) {
        return (
            <AuthenticatedLayout
                user={auth.user}
                header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Learning</h2>}
            >
                <div className="py-12">
                    <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                        <div className="text-center">
                            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-gray-900 dark:border-gray-100 mx-auto"></div>
                            <p className="mt-2 text-gray-600 dark:text-gray-400">Loading course content...</p>
                        </div>
                                    </div>
            </div>
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Learning: {course.title}</h2>}
        >
                ...
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        <User className={`w-5 h-5 ${
                                            isEnrolled ? 'text-blue-600 dark:text-blue-400' : 'text-orange-600 dark:text-orange-400'
                                        }`} />
                                        <div>
                                            <div className={`font-medium ${
                                                isEnrolled ? 'text-blue-900 dark:text-blue-100' : 'text-orange-900 dark:text-orange-100'
                                            }`}>
                                                Viewing as {viewingContact.first_name} {viewingContact.last_name}
                                            </div>
                                            <div className={`text-sm ${
                                                isEnrolled ? 'text-blue-700 dark:text-blue-300' : 'text-orange-700 dark:text-orange-300'
                                            }`}>
                                    ...
                                    <div className="flex gap-2">
                                        {!isEnrolled && !isCheckingEnrollment && (
                                            <Link href={`/courses/${course.id}?contact_id=${viewingContact.id}`}>
                                                <Button 
                                                    variant="outline" 
                                                    size="sm"
                                        ...
                                        {isEnrolled && !isCheckingEnrollment && (
                                            <Button 
                                                ...
                                                size="sm"
                                                disabled
                                                className="text-green-600 dark:text-green-400 border-green-300 dark:border-green-600 bg-green-50 dark:bg-green-900/20 cursor-not-allowed"
                                            >
                                                Already Enrolled
                                            </Button>
                                        )}
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <div className="text-sm text-yellow-700 dark:text-yellow-300">
                                                Select a contact to track lesson progress
                                            </div>
                                        </div>
                                    </div>
                                    <Button 
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                            ...
                            <Card>
                                ...
                                <CardContent>
                                    {currentLesson ? (
                                        <div>
                                            <div className="mb-6">
                                                <h3 className="text-xl font-semibold mb-2 text-gray-900 dark:text-gray-100">{currentLesson.title}</h3>
                                                <p className="text-gray-600 dark:text-gray-400 mb-4">{currentLesson.description}</p>
                                            ...
                                            <div className="flex justify-between items-center pt-4 border-t">
                                                ...
                                                >
                                                    <Play className="w-4 h-4 mr-2" />
                                                    {currentLesson.status === 'not_started' ? 'Mark as Started' : 
                                                     currentLesson.status === 'in_progress' ? 'In Progress' : 
                                                     currentLesson.status === 'completed' ? 'Completed' : 'Mark as Started'}
                                                </Button>
                        ...
                        <div className="lg:col-span-1">
                            <Card>
                                <CardHeader>
                                    <CardTitle className="text-gray-900 dark:text-gray-100">Course Progress</CardTitle>
                                </CardHeader>
                                <CardContent className="space-y-4">
                                    {currentProgress && (
                                        <div>
                                            ...
                                            <Progress value={currentProgress.progress_percentage} className="mb-4" />
                                            <div className="text-sm text-gray-600 dark:text-gray-400">
                                                {currentProgress.lessons_completed} of {currentProgress.total_lessons} lessons completed
                                            </div>
                                        </div>
                                    )}

                                    <div className="border-t dark:border-gray-700 pt-4">
                                        <h4 className="font-medium mb-3 text-gray-900 dark:text-gray-100">Lessons</h4>
                                                ...
                                                >
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div className="flex-1">
                                                            <div className="font-medium text-sm text-gray-900 dark:text-gray-100">{lesson.title}</div>
                                    ...
                                    {currentProgress?.progress_percentage === 100 && (
                                        <div className="border-t dark:border-gray-700 pt-4">
                                            <div className="text-center">
                                                ...
                                                <h4 className="font-medium text-green-600 dark:text-green-400">Course Completed!</h4>
