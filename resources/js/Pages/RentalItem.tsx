...
import { Plus, Edit, Trash, Search, History } from "lucide-react";
...
export default function RentalItem({ auth, initialItems, initialPayments, appCurrency }: Props) {
    ...
    const handleSaveItem = async (e: React.FormEvent) => {
        ...
        try {
            if (currentItem) {
                if (currentItem.id) {
                    // Edit existing item
                    await axios.put(`/rental-item/${currentItem.id}`, currentItem);
                } else {
                    // Add new item
    ...
    const handlePaymentHistory = async (item: RentalItem) => {
        try {
            const response = await axios.get(`/rental-item/${item.id}/payment-history`);
            ...
            <div className="p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
                            ...
                            {paginatedItems.map((item) => (
                                ...
                                <TableCell>
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-500">{item.renter_contact}</p>
                                        <p className="text-sm text-gray-500">{item.rental_start} to {item.rental_end}</p>
                                    </div>
                                    ) : (
                                    "N/A"
                                    )}
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        {item.status === 'rented' && (
                                            <>
                                                ...
                                                <Button variant="outline" size="sm" onClick={() => handlePaymentHistory(item)}>
                    ...
                    <Dialog open={isItemDialogOpen} onOpenChange={setIsItemDialogOpen}>
                        ...
                        <form onSubmit={handleSaveItem}>
                            ...
                            <div className="grid grid-cols-4 items-center gap-4">
                                <Label htmlFor="daily_rate" className="text-right">Daily Rate</Label>
                                <Input
                                id="daily_rate"
                                type="number"
                                value={currentItem?.daily_rate || ''}
                                onChange={(e) => setCurrentItem({ ...currentItem!, daily_rate: parseFloat(e.target.value) })}
                                className="col-span-3"
                                />
                            </div>
                            <div className="grid grid-cols-4 items-center gap-4">
                            ...
                            {currentItem?.status === 'rented' && (
                                ...
                                <div className="grid grid-cols-4 items-center gap-4">
                                    ...
                                    <Input
                                    id="renterContact"
                                    value={currentItem?.renter_contact || ''}
                                    onChange={(e) => setCurrentItem({ 
                                        ...currentItem!, 
                                ...
                                <div className="grid grid-cols-4 items-center gap-4">
                                    ...
                                    id="rentalStart"
                                    type="date"
                                    value={currentItem?.rental_start || ''}
                                    onChange={(e) => setCurrentItem({ 
                                        ...currentItem!, 
                        ...
                        <DialogContent>
                            ...
                            <form onSubmit={handlePaymentSubmit}>
                                <div className="grid gap-4 py-4">
                                    ...
                                    <div className="grid grid-cols-4 items-center gap-4">
                                        <Label htmlFor="renter_name" className="text-right">Renter</Label>
                                        <div className="col-span-3">
                            ...
                            <div className="space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <p className="text-sm">{selectedItemHistory?.name}</p>
                                    </div>
                                    <div>
                                        <Label>Renter Name</Label>
                                        <p className="text-sm">{selectedItemHistory?.renter_name}</p>
                                    </div>
                                    <div>
                                        <Label>Rental Period</Label>
                                        <p className="text-sm">
                                            {selectedItemHistory?.rental_start} to {selectedItemHistory?.rental_end}
                                        </p>
                                    </div>
                                    <div>
