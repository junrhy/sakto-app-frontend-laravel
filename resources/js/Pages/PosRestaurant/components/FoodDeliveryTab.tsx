...
interface FoodDeliveryTabProps {
    ...
    canDelete: boolean;
    onAddOnlineStore?: (
        storeData: Omit<OnlineStore, 'id' | 'created_at' | 'updated_at'>,
    ) => void;
...
export const FoodDeliveryTab: React.FC<FoodDeliveryTabProps> = ({
    ...
    const [isDeleteStoreDialogOpen, setIsDeleteStoreDialogOpen] =
        useState(false);
    const [isMenuItemsDialogOpen, setIsMenuItemsDialogOpen] = useState(false);
    const [editStoreData, setEditStoreData] = useState<OnlineStore | null>(
        null,
    );
    ...
    const [onlineStoresData, setOnlineStoresData] =
        useState<OnlineStore[]>(onlineStores);
    ...
    const handleSelectStore = useCallback(
        (storeId: number, checked: boolean) => {
            if (checked) {
                                            ...
                                            <TableCell className="text-gray-900 dark:text-white">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <button
                                                        onClick={() =>
                                                            window.open(
                                                                `/fnb/store/${store.domain}`,
                                                                '_blank',
                                                            )
                                                        }
                                                        className="cursor-pointer font-mono text-sm text-blue-600 transition-colors hover:text-blue-800 hover:underline"
...
const ManageMenuItemsDialog: React.FC<ManageMenuItemsDialogProps> = ({
    ...
    const isIndeterminate =
        ...
        selectedMenuItems.length < menuItems.length;
    ...
    return (
        <Dialog open={isOpen} onOpenChange={handleClose}>
            <DialogContent className="sm:max-w-[600px]">
                <DialogHeader>
                    <DialogTitle>
                        Manage Menu Items for "{store?.name}"
                    </DialogTitle>
                    <DialogDescription>
                        Select which menu items will be available in this online
                        store.
                    </DialogDescription>
                </DialogHeader>
                <form onSubmit={handleSubmit} className="space-y-4">
