...
export const BlockedDatesTab: React.FC<BlockedDatesTabProps> = ({
    ...
    const datesWithBlocks = useMemo(() => {
        return Array.from(
            new Set(
                optimisticBlockedDates.map(
                    (blocked) => blocked.blocked_date.split('T')[0],
                ),
            ),
        );
    ...
    const handleTimeSlotToggle = useCallback(
        async (timeSlot: string) => {
            ...
            if (openedDates.length > 0) {
                ...
                if (!isOpened) {
                    toast.error(
                        'This time slot is not opened. Please open it first in the "Opened Dates" tab before blocking.',
                    );
                    return;
                }
            }
                ...
                if (alreadyBlockedEntry) {
                    ...
                    if (remainingSlots.length === 0) {
                        ...
                        await onUpdateBlockedDate(alreadyBlockedEntry.id, {
                            blocked_date:
                                alreadyBlockedEntry.blocked_date.split('T')[0],
                            timeslots: remainingSlots,
                            client_identifier:
                                alreadyBlockedEntry.client_identifier,
                        });
                    }
                } else {
                    // BLOCK: Add this slot
