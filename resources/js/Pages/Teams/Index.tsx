...
import { Plus, Search, MoreHorizontal, Eye, Edit, Trash2, UserCheck, UserX, Key, Settings } from 'lucide-react';
...
export default function Index({ auth, teamMembers, availableRoles, availableApps }: Props) {
  ...
  const handleResetPassword = async (identifier: string) => {
    try {
      const response = await fetch(`/teams/${identifier}/reset-password`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '',
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-blue-600">
                  {teamMembers.filter(m => m.email_verified).length}
                </div>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-orange-600">
                  {teamMembers.filter(m => m.last_logged_in && new Date(m.last_logged_in) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)).length}
                </div>
              </CardContent>
            </Card>
          </div>
                  ...
                  <TableBody>
                    {filteredMembers.map((member) => (
                      <TableRow key={member.identifier}>
                        ...
                        <TableCell>
                          <div className="flex flex-wrap gap-1">
                            {member.allowed_apps.slice(0, 3).map((app) => (
                              <Badge key={app} variant="secondary" className="text-xs">
                                {availableApps[app] || app}
                              </Badge>
                            ))}
                            {member.allowed_apps.length > 3 && (
                              <Badge variant="secondary" className="text-xs">
                                +{member.allowed_apps.length - 3} more
                              </Badge>
                            )}
                          </div>
                        </TableCell>
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              <DropdownMenuItem asChild>
                                <Link href={route('teams.show', member.identifier)}>
                                  <Eye className="w-4 h-4 mr-2" />
                                  View Details
                                </Link>
                              </DropdownMenuItem>
                              <DropdownMenuItem asChild>
                                <Link href={route('teams.edit', member.identifier)}>
                                  <Edit className="w-4 h-4 mr-2" />
                                  Edit
                                </Link>
                              </DropdownMenuItem>
                              <DropdownMenuSeparator />
                              <DropdownMenuItem onClick={() => handleToggleStatus(member.identifier)}>
                                {member.is_active ? (
                                  <>
                              ...
                              <DropdownMenuItem onClick={() => handleResetPassword(member.identifier)}>
                                <Key className="w-4 h-4 mr-2" />
                                Reset Password
                              </DropdownMenuItem>
                              <DropdownMenuSeparator />
                              <DropdownMenuItem 
                                onClick={() => handleDelete(member.identifier)}
