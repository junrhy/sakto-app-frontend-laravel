...
export function ClinicPaymentAccountManager({
    ...
    const [selectedAccount, setSelectedAccount] =
        useState<ClinicPaymentAccount | null>(null);
    ...
    const [showingHistoryForPatient, setShowingHistoryForPatient] =
        useState<Patient | null>(null);
    const [activeHistoryType, setActiveHistoryType] = useState<
        'bill' | 'payment'
    >('bill');
    ...
    const handleDeleteAccount = async (accountId: number) => {
        ...
        try {
            const response = await axios.delete(
                `/clinic/payment-accounts/${accountId}`,
            );
            if (response.data.success) {
                setAccounts((prev) =>
                ...
                toast.error(
                    response.data.message || 'Failed to delete payment account',
                );
            }
        } catch (error: any) {
            ...
            if (error.response?.status === 400) {
                toast.error(
                    error.response.data.message ||
                        'Cannot delete account with associated data',
                );
    ...
    const handleViewDetails = async (account: ClinicPaymentAccount) => {
        ...
        try {
            const response = await axios.get(
                `/clinic/payment-accounts/${account.id}`,
            );
    ...
    const handleAddBill = async (e: React.FormEvent) => {
        ...
        const result = await onAddBill(
            selectedPatientForBilling.id,
            amount,
            billDetails,
            billDate,
        );
    ...
    const groupCount = accounts.filter(
        (acc) => acc.account_type === 'group',
    ).length;
    const companyCount = accounts.filter(
        (acc) => acc.account_type === 'company',
    ).length;
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground">
                        Manage individual patient billing and group/company
                        payment accounts
                    </p>
                </div>
                <Dialog
            ...
            >
                <TabsList className="mb-6 grid h-auto w-full grid-cols-3 rounded-xl border border-gray-200 bg-gray-100 p-1 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    ...
                    >
                        ...
                        <Badge
                            variant="secondary"
                            className="ml-1 bg-gray-200 px-2 py-0.5 text-xs text-gray-700 dark:bg-gray-600 dark:text-gray-300"
                        >
                            {patientCount}
                        </Badge>
                    </TabsTrigger>
                ...
                <TabsContent value="patient-billing" className="mt-0 space-y-6">
                    <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row">
                        <div>
                            ...
                            <p className="mt-1 text-muted-foreground">
                                Manage bills and payments for individual
                                patients
                            </p>
                        </div>
                ...
                <TabsContent value="group" className="mt-0 space-y-6">
                    {loading ? (
                        ...
                        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                            {filteredAccounts
                                .filter((acc) => acc.account_type === 'group')
                                .map((account) => (
                                    <Card
                                        key={account.id}
                ...
                <TabsContent value="company" className="mt-0 space-y-6">
                    {loading ? (
                        ...
                        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                            {filteredAccounts
                                .filter((acc) => acc.account_type === 'company')
                                .map((account) => (
                                    <Card
                                        key={account.id}
                                        className="transition-shadow hover:shadow-md"
                    ...
                    <DialogHeader>
                        <DialogTitle>
                            ...
                            {selectedPatientForBilling &&
                                `for ${selectedPatientForBilling.name}`}
                        </DialogTitle>
                    </DialogHeader>
                        ...
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                            ...
                            <div>
                                <Label htmlFor="billAmount">
                                    Bill Amount ({appCurrency?.symbol || '$'}) *
                                </Label>
                                <Input
                                    id="billAmount"
                                    type="number"
                    ...
                    <DialogHeader>
                        <DialogTitle>
                            ...
                            {selectedPatientForPayment &&
                                `for ${selectedPatientForPayment.name}`}
                        </DialogTitle>
                    </DialogHeader>
                        ...
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                            ...
                            <div>
                                <Label htmlFor="paymentAmount">
                                    Payment Amount ({appCurrency?.symbol || '$'}
                                    ) *
                                </Label>
                                <Input
                                    id="paymentAmount"
            ...
            <HistoryDialog
                ...
                onDeleteBill={
                    onDeleteBill
                        ? (patientId: string, billId: number) => {
                ...
                onDeletePayment={
                    onDeletePayment
                        ? (patientId: string, paymentId: number) => {
                              onDeletePayment(patientId, paymentId).then(
                                  (result) => {
                                      if (result.success) {
                                          toast.success(
