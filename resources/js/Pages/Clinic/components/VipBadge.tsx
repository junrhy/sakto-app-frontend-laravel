...
const VipBadge: React.FC<VipBadgeProps> = ({ 
    ...
    const tierConfig = patient.vip_tier_display || getVipTierConfig(patient.vip_tier || 'standard');
    ...
    if (patient.extended_consultation_time) privileges.push('Extended Consultation');
    if (patient.dedicated_staff_assignment) privileges.push('Dedicated Staff');
    if (patient.complimentary_services) privileges.push('Complimentary Services');
    if (patient.vip_discount_percentage && patient.vip_discount_percentage > 0) {
                    ...
                    <div className="space-y-2">
                        ...
                        {patient.vip_since && (
                            <p className="text-xs text-gray-500 dark:text-gray-400">
                                VIP since: {new Date(patient.vip_since).toLocaleDateString()}
                            </p>
                        )}
                        {privileges.length > 0 && (
                            <div>
