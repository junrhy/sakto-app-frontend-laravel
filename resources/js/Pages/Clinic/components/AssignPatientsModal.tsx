...
export function AssignPatientsModal({ 
    ...
    const assignedPatients = patients.filter(p => p.clinic_payment_account_id === account.id);
    const unassignedPatients = patients.filter(p => !p.clinic_payment_account_id || p.clinic_payment_account_id !== account.id);
    ...
    const handleAssignPatients = async () => {
        ...
        setIsAssigning(true);
        try {
            const response = await axios.post(`/clinic/payment-accounts/${account.id}/assign-patients`, {
                patient_ids: selectedPatients.map(id => parseInt(id))
            });

            if (response.data.success) {
                toast.success(`${selectedPatients.length} patient(s) assigned successfully`);
                setSelectedPatients([]);
                onPatientsAssigned();
            }
        } catch (error: any) {
            console.error('Failed to assign patients:', error);
            toast.error(error.response?.data?.message || 'Failed to assign patients');
        } finally {
    ...
    const handleRemovePatients = async () => {
        ...
        setIsRemoving(true);
        try {
            const response = await axios.post(`/clinic/payment-accounts/${account.id}/remove-patients`, {
                patient_ids: selectedPatients.map(id => parseInt(id))
            });

            if (response.data.success) {
                toast.success(`${selectedPatients.length} patient(s) removed successfully`);
                setSelectedPatients([]);
                onPatientsAssigned();
            }
        } catch (error: any) {
            console.error('Failed to remove patients:', error);
            toast.error(error.response?.data?.message || 'Failed to remove patients');
        } finally {
            setIsRemoving(false);
        }
    };
    ...
    const PatientList = ({ patientList, showAssignedBadge = false }: { patientList: Patient[], showAssignedBadge?: boolean }) => (
            ...
            <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    ...
                    <TabsContent value="assign" className="space-y-4">
                        <Card>
                            <CardHeader>
                                <CardTitle className="text-lg">Available Patients</CardTitle>
                                <CardDescription>
                                ...
                                {filteredUnassigned.length > 0 && (
                                    <div className="flex space-x-2">
                                        <Button 
                                            variant="outline" 
                                            size="sm"
                                            onClick={() => handleSelectAll(filteredUnassigned)}
                                        >
                                            Select All
                                        </Button>
                                        <Button 
                                ...
                                {selectedPatients.length > 0 && (
                                    <div className="mt-4 pt-4 border-t">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <Button onClick={handleAssignPatients} disabled={isAssigning}>
                    ...
                    <TabsContent value="manage" className="space-y-4">
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>
                                    Patients currently assigned to {account.account_name}
                                </CardDescription>
                                {filteredAssigned.length > 0 && (
                                    <div className="flex space-x-2">
                                        <Button 
                                            variant="outline" 
                                            size="sm"
                                            onClick={() => handleSelectAll(filteredAssigned)}
                                        >
                                            Select All
                                        </Button>
                                        <Button 
                            ...
                            <CardContent>
                                <PatientList patientList={filteredAssigned} showAssignedBadge />
