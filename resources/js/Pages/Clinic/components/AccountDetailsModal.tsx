...
import { Building2, Users, Mail, Phone, MapPin, CreditCard, Calendar, DollarSign, Edit2, Check, X } from 'lucide-react';
import { ClinicPaymentAccount, AppCurrency } from '../types';
...
export function AccountDetailsModal({ 
    ...
    const handleBillStatusUpdate = async (billId: number, newStatus: string) => {
        setUpdatingBillId(billId);
        try {
            const response = await axios.put(`/clinic/patient-bills/${billId}/status`, {
            ...
            if (response.data.success) {
                ...
                const updatedAccount = {
                    ...
                    bills: account.bills?.map(bill => 
                        bill.id === billId ? { ...bill, bill_status: newStatus } : bill
                    )
                };
                                ...
                                <div className="grid gap-4 md:grid-cols-4">
                                    <div className="text-center p-4 border rounded-lg">
                                        <p className="text-2xl font-bold text-blue-600">
                                            {formatCurrency(
                                                account.bills?.reduce((sum: number, bill: any) => 
                                                    sum + (parseFloat(bill.bill_amount) || 0), 0
                                    ...
                                    <div className="text-center p-4 border rounded-lg">
                                        <p className="text-2xl font-bold text-green-600">
                                            {formatCurrency(
                                                account.payments?.reduce((sum: number, payment: any) => 
                                    ...
                                    <div className="text-center p-4 border rounded-lg">
                                        <p className="text-2xl font-bold text-red-600">
                                            {formatCurrency((() => {
                                                const totalBills = account.bills?.reduce((sum: number, bill: any) => 
                                                    sum + (parseFloat(bill.bill_amount) || 0), 0
                                                ) || account.total_bills || 0;
                                                const totalPayments = account.payments?.reduce((sum: number, payment: any) => 
                                                    sum + (parseFloat(payment.payment_amount) || 0), 0
                                                ) || account.total_payments || 0;
                                                return Math.max(0, totalBills - totalPayments);
                                            })())}
                                        </p>
                                ...
                                {account.patients && account.patients.length > 0 ? (
                                        ...
                                        <TableBody>
                                            {account.patients.map((patient) => (
                                                <TableRow key={patient.id}>
                                                    ...
                                                    <TableCell className="font-mono">{patient.arn}</TableCell>
                                                    <TableCell>{patient.email}</TableCell>
                                                    <TableCell>{patient.phone}</TableCell>
                                                    <TableCell>
                                                        <Badge variant={patient.billing_type === 'account' ? 'default' : 'secondary'}>
                                    ...
                                    <div className="text-center py-8">
                                        ...
                                        <p className="text-muted-foreground">
                                            Assign patients to this account to start group billing
                                        </p>
                                    </div>
                                )}
                            </CardContent>
                                ...
                                {account.bills && account.bills.length > 0 ? (
                                    <div className="space-y-4">
                                        <Table>
                                            ...
                                            <TableBody>
                                                {account.bills.map((bill: any) => (
                                                    <TableRow key={bill.id}>
                                                        <TableCell className="font-mono">
                                                            {bill.account_bill_reference || bill.bill_number || `BILL-${bill.id}`}
                                                        </TableCell>
                                                        <TableCell>
                                                        ...
                                                        <TableCell className="font-medium">
                                                            {formatCurrency(parseFloat(bill.bill_amount) || 0)}
                                                        </TableCell>
                                                        <TableCell>
                                                            {editingBillId === bill.id ? (
                                                                <div className="flex items-center space-x-2">
                                                                    <Select 
                                                                        value={bill.bill_status || 'pending'}
                                                                        onValueChange={(newStatus) => handleBillStatusUpdate(bill.id, newStatus)}
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <SelectContent>
                                                                            {getBillStatusOptions().map(option => (
                                                                ...
                                                                <Badge variant={
                                                                    bill.bill_status === 'paid' ? 'default' : 
                                                                    bill.bill_status === 'overdue' ? 'destructive' : 
                                                                    bill.bill_status === 'partial' ? 'secondary' :
                                                                    'secondary'
                                                                }>
                                        ...
                                        <div className="pt-4 border-t">
                                            <div className="flex justify-between text-sm">
                                                ...
                                                <span className="font-medium">
                                                    {formatCurrency(account.bills.reduce((sum: number, bill: any) => 
                                                        sum + (parseFloat(bill.bill_amount) || 0), 0
                                                    ))}
                                                </span>
                                            </div>
                                    ...
                                    <div className="text-center py-8">
                                        ...
                                        <p className="text-muted-foreground">
                                            Create bills for this account to see them here
                                        </p>
                                    </div>
                                )}
                            </CardContent>
                                ...
                                {account.payments && account.payments.length > 0 ? (
                                    <div className="space-y-4">
                                        <Table>
                                            ...
                                            <TableBody>
                                                {account.payments.map((payment: any) => (
                                                    <TableRow key={payment.id}>
                                                        <TableCell className="font-mono">
                                                            {payment.account_payment_reference || `PAY-${payment.id}`}
                                                        </TableCell>
                                                        <TableCell>
                                                        ...
                                                        <TableCell>
                                                            <Badge variant="outline">
                                                                {payment.covered_patients?.length || 1} patient(s)
                                                            </Badge>
                                                        </TableCell>
                                                    </TableRow>
                                                ))}
                                        ...
                                        <div className="pt-4 border-t">
                                            <div className="flex justify-between text-sm">
                                                ...
                                                <span className="font-medium text-green-600">
                                                    {formatCurrency(account.payments.reduce((sum: number, payment: any) => 
                                                        sum + (parseFloat(payment.payment_amount) || 0), 0
                                                    ))}
                                                </span>
                                            </div>
                                    ...
                                    <div className="text-center py-8">
                                        ...
                                        <p className="text-muted-foreground">
                                            Record payments for this account to see them here
                                        </p>
                                    </div>
                                )}
                            </CardContent>
