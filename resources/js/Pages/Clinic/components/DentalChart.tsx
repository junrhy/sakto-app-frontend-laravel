...
const DentalChart: React.FC<DentalChartProps> = ({
    ...
    const getToothImage = (status: ToothStatus) => {
        switch (status) {
            case 'healthy':
                return '/images/healthy-tooth.svg';
            case 'decayed':
                return '/images/decayed-tooth.svg';
            case 'filled':
                return '/images/filled-tooth.svg';
            case 'missing':
                return '/images/missing-tooth.svg';
            default:
                return '/images/healthy-tooth.svg';
        }
    };
    ...
    const renderToothRow = useMemo(
        () => (teeth: ToothData[], isUpper: boolean) => (
            <div className="flex justify-center space-x-1">
                {teeth.map((tooth, index) => {
                    ...
                    return (
                        <div
                            key={tooth.id}
                            className="relative h-12 w-8 cursor-pointer"
                        ...
                        >
                            ...
                            />
                            <span className="absolute bottom-0 left-1/2 -translate-x-1/2 transform text-xs">
                                {toothNumber}
                            </span>
                        </div>
                    );
    ...
    const renderLegend = useMemo(
        () => () => (
            <div className="mt-4 flex justify-center space-x-4">
                {(
                    ['healthy', 'decayed', 'filled', 'missing'] as ToothStatus[]
                ).map((status) => (
