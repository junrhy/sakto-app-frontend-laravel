...
interface AgricultureProps extends PageProps {
    agriculture: AgricultureProvider[];
}

export default function Agriculture({ auth, agriculture }: AgricultureProps) {
    const [searchQuery, setSearchQuery] = useState('');

    const filteredAgriculture = agriculture.filter(
        (provider) =>
        ...
        >
            ...
            <div className="overflow-hidden border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800 sm:rounded-lg">
                <div className="p-4 text-gray-900 dark:text-gray-100 sm:p-6">
                    <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div className="flex-shrink-0">
                            ...
                            <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                                Total: {filteredAgriculture.length}{' '}
                                {filteredAgriculture.length === 1
                                    ? 'provider'
                                    : 'providers'}
                            </p>
                        </div>
                    ...
                    {filteredAgriculture.length > 0 ? (
                        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                            {filteredAgriculture.map((provider) => (
                                <Card
                                    key={provider.id}
                        ...
                        <div className="rounded-lg border border-gray-200 bg-white p-6 text-center shadow-sm dark:border-gray-700 dark:bg-gray-800 sm:p-8">
                            ...
                            <h3 className="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100 sm:text-base">
                                {searchQuery
                                    ? 'No providers found'
                                    : 'No agriculture providers available'}
                            </h3>
                            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400 sm:text-sm">
                                {searchQuery
                                    ? 'Try adjusting your search terms.'
                                    : 'No agriculture providers have been added yet.'}
                            </p>
                        </div>
                    )}
                </div>
