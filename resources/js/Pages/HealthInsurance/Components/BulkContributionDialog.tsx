...
export default function BulkContributionDialog({
    ...
    const groups = [
        'all',
        ...Array.from(new Set(members.map((m) => m.group).filter(Boolean))),
    ];
    ...
    const handleMemberSelection = (
        ...
        setMemberContributions((prev) =>
            prev.map((member) => ({
                ...member,
                selected:
                    member.member_id === memberId ? selected : member.selected,
                amount:
                    member.member_id === memberId &&
                    selected &&
                    formData.bulk_amount
                        ? Number(formData.bulk_amount)
                        : member.amount,
            })),
        );
        ...
        const newSelectedMembers = selected
            ...
            : formData.selected_members.filter(
                  (id: string | number) => id !== memberId,
              );
        setFormData((prev) => ({
            ...prev,
            selected_members: newSelectedMembers,
        }));
    };
    ...
    const handleSelectAll = (selected: boolean) => {
        ...
        setMemberContributions((prev) =>
            prev.map((member) => ({
                ...member,
                selected: allMemberIds.includes(member.member_id)
                    ? selected
                    : member.selected,
                amount:
                    allMemberIds.includes(member.member_id) &&
                    selected &&
                    formData.bulk_amount
                        ? Number(formData.bulk_amount)
                        : member.amount,
        ...
        setFormData((prev) => ({
            ...prev,
            selected_members: selected ? allMemberIds : [],
        }));
    };
    ...
    const handleMemberDateChange = (
        ...
        setMemberContributions((prev) =>
            prev.map((member) => ({
                ...member,
                payment_date:
                    member.member_id === memberId ? date : member.payment_date,
            })),
        );
    };

    const handleIndividualDateToggle = (
        memberId: string | number,
        ...
        setMemberContributions((prev) =>
            prev.map((member) => ({
                ...
                use_individual_date:
                    member.member_id === memberId
                        ? useIndividual
                        : member.use_individual_date,
                payment_date:
                    member.member_id === memberId && !useIndividual
    ...
    const handleMultipleDatesToggle = (
        memberId: string | number,
        useMultiple: boolean,
    ) => {
        setMemberContributions((prev) =>
            prev.map((member) => {
                if (member.member_id === memberId) {
                    return {
                        ...
                        use_individual_date: useMultiple
                            ? false
                            : member.use_individual_date,
                        multiple_contributions: useMultiple
                            ? [
                                  {
    ...
    const removeMultipleContribution = (
        memberId: string | number,
        index: number,
    ) => {
        setMemberContributions((prev) =>
            prev.map((member) => {
                if (member.member_id === memberId) {
                    return {
                        ...member,
                        multiple_contributions:
                            member.multiple_contributions.filter(
                                (_, i) => i !== index,
                            ),
                    };
                }
    ...
    const updateMultipleContribution = (
        memberId: string | number,
        index: number,
        field: 'date' | 'amount',
        value: string | number,
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        memberContributions
            ...
            .forEach((member) => {
                if (
                    member.use_multiple_dates &&
                    member.multiple_contributions.length > 0
                ) {
                    ...
                    selectedContributions.push({
                        ...
                        payment_date: member.use_individual_date
                            ? member.payment_date
                            : formData.payment_date,
                        payment_method: formData.payment_method,
        ...
        if (selectedContributions.length === 0) {
            setErrors({
                general:
                    'Please select at least one member with a valid amount',
            });
            return;
        }
        ...
        fetch(route('health-insurance.contributions.bulk'), {
            ...
            .then((data) => {
                ...
                if (data.success) {
                    ...
                    const successfulContributions = selectedContributions.map(
                        (c) => ({
                            ...c,
                            id: '',
                        }),
                    ...
                    console.log(
                        `Successfully recorded ${selectedContributions.length} contributions`,
                    );
                } else {
                    throw new Error(
                        data.message || 'Failed to record contributions',
                    );
                }
            })
            .catch((error) => {
                ...
                // Show error message
                setErrors({
                    general: error.message || 'Failed to record contributions',
                });
            });
    };
    ...
    const totalAmount = memberContributions
        ...
        .reduce((sum, m) => {
            if (m.use_multiple_dates && m.multiple_contributions.length > 0) {
                return (
                    sum +
                    m.multiple_contributions.reduce(
                        (memberSum, contribution) =>
                            memberSum + (Number(contribution.amount) || 0),
                        0,
                    )
                );
            }
    ...
    const totalContributions = memberContributions
        ...
        .reduce((sum, m) => {
            if (m.use_multiple_dates && m.multiple_contributions.length > 0) {
                return (
                    sum +
                    m.multiple_contributions.filter((c) => c.amount > 0).length
                );
            }
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="flex max-h-[90vh] flex-col sm:max-w-[1400px]">
                ...
                >
                                    ...
                                    <CardContent className="space-y-3">
                                        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                                            ...
                                            <div>
                                                ...
                                                >
                                                    <SelectTrigger
                                                        className={`h-9 ${errors.payment_method ? 'border-red-500' : ''}`}
                                                    >
                                    ...
                                    <CardContent>
                                                        ...
                                                        (
                                                            ...
                                                            return (
                                                                <div
                                                                    ...
                                                                    className="rounded-lg border border-gray-200 bg-gray-50 p-3 hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800/50 dark:hover:bg-gray-700/50"
                                                                >
                                                                    ...
                                                                    {!memberContribution.use_multiple_dates && (
                                                                        <div className="space-y-2">
                                                                            <div className="flex items-center gap-2">
                                                                                ...
                                                                                />
                                                                                <Label
                                                                                    htmlFor={`individual-date-${member.id}`}
                                                                                    className="text-xs"
                                                                                >
                                                                                    Use
                                                                                    individual
                                                                                ...
                                                                                {memberContribution.use_individual_date && (
                                                                                    <Input
                                                                                        type="date"
                                                                                        value={
                                                                                            memberContribution.payment_date
                                                                                        }
                                                                                        onChange={(
                                                                                            e,
                                                                                        ) =>
                                                                    ...
                                                                    <div className="space-y-2">
                                                                        <div className="flex items-center gap-2">
                                                                            ...
                                                                            />
                                                                            <Label
                                                                                htmlFor={`multiple-dates-${member.id}`}
                                                                                className="text-xs"
                                                                            >
                                                                                Multiple
                                                                                dates
                                                                        ...
                                                                        {memberContribution.use_multiple_dates && (
                                                                            <div className="ml-5 space-y-2">
                                                                                {memberContribution.multiple_contributions.map(
                                                                                    (
                                                                                        contribution,
                                                                                        index,
                                                                                    ) => (
                                                                                        <div
                                                                                            ...
                                                                                            className="flex items-center gap-2 rounded border bg-white p-2 dark:bg-gray-700"
                                                                                        >
                                                                                            <Input
                                                                                                type="date"
                                                                                                value={
                                        ...
                                        {selectedCount > 0 && (
                                            <div className="mt-4 rounded-lg border border-green-200 bg-green-50 p-3 dark:border-green-800 dark:bg-green-900/20">
                                                ...
                                                <div className="mt-1 flex items-center justify-between text-xs text-green-600 dark:text-green-400">
                                                    <span>
                                                        Total Contributions:
                                                    </span>
                                                    <span>
                                                ...
                                                <Button
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        setFormData((prev) => ({
                                                            ...prev,
                                                            selected_members: [
                                                                ...prev.selected_members,
                                                                ...groupMembers.filter(
                                                                    (id) =>
                                                                        !prev.selected_members.includes(
                                                                            id,
                                                                        ),
                                                                ),
                                                            ],
                    ...
                    <DialogFooter className="flex flex-shrink-0 flex-col gap-2 pt-4 sm:flex-row">
                        <Button
                            type="button"
                            variant="outline"
