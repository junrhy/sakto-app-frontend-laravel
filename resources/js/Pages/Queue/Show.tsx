...
export default function QueueShow({ auth, queueType }: Props) {
    ...
    const getStatusCount = (status: string) => {
        return queueType.queue_numbers.filter((qn) => qn.status === status)
            .length;
    };
    ...
    const callNext = async () => {
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                if (
                    data.message &&
                    data.message.includes('No waiting queue numbers found')
                ) {
    ...
    const updateQueueStatus = async (queueNumberId: number, action: string) => {
        ...
        try {
            const response = await fetch(
                route(`queue.${action}`, queueNumberId),
                {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                ...
                <div className="mx-auto max-w-7xl sm:px-6 lg:px-8">
                        ...
                        >
                            ...
                            <DialogContent className="sm:max-w-[425px]">
                                <DialogHeader>
                                    ...
                                    <DialogDescription>
                                        Add a new customer to the{' '}
                                        {queueType.name} queue. Customer
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Manage and track all queue numbers for this
                                queue type
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                            ...
                            >
                                ...
                                <TabsContent value={activeTab} className="mt-6">
                                    <div className="space-y-4">
                                        ...
                                        {getFilteredQueueNumbers().length ===
                                            0 && (
