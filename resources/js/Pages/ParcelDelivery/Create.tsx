...
export default function ParcelDeliveryCreate({ auth }: Props) {
    const { data, setData, post, processing, errors } = useForm<ParcelDeliveryFormData>({
    ...
    const calculatePricing = async () => {
        if (!data.package_weight || !data.distance_km || !data.pickup_date || !data.pickup_time) {
            toast.error('Please fill in required fields for pricing calculation');
            return;
        }

        setCalculating(true);
        try {
            const response = await axios.post('/parcel-delivery/calculate-pricing', {
                delivery_type: data.delivery_type,
                package_weight: parseFloat(data.package_weight),
                distance_km: parseFloat(data.distance_km),
                package_length: data.package_length ? parseFloat(data.package_length) : null,
                package_width: data.package_width ? parseFloat(data.package_width) : null,
                package_height: data.package_height ? parseFloat(data.package_height) : null,
                package_value: data.package_value ? parseFloat(data.package_value) : null,
                                ...
                                <CardContent className="space-y-4">
                                    <div>
                                        ...
                                        <Select
                                            ...
                                            onValueChange={(value) => setData('delivery_type', value as any)}
                                        >
                                            <SelectTrigger>
                                                <SelectValue />
                                            </SelectTrigger>
                                        ...
                                        {errors.delivery_type && (
                                            <p className="text-sm text-red-600 mt-1">{errors.delivery_type}</p>
                                        )}
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            />
                                            {errors.pickup_date && (
                                                <p className="text-sm text-red-600 mt-1">{errors.pickup_date}</p>
                                            )}
                                        </div>
                                        <div>
                            ...
                            <Card>
                                ...
                                <CardContent className="space-y-4">
                                    <div>
                                        ...
                                        />
                                        {errors.sender_name && (
                                            <p className="text-sm text-red-600 mt-1">{errors.sender_name}</p>
                                        )}
                                    </div>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            />
                                            {errors.sender_phone && (
                                                <p className="text-sm text-red-600 mt-1">{errors.sender_phone}</p>
                                            )}
                                        </div>
                                        <div>
                                    ...
                                    <div>
                                        ...
                                        {errors.sender_address && (
                                            <p className="text-sm text-red-600 mt-1">{errors.sender_address}</p>
                                        )}
                                    </div>
                                </CardContent>
                            </Card>
                                ...
                                <CardContent className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            {errors.recipient_phone && (
                                                <p className="text-sm text-red-600 mt-1">{errors.recipient_phone}</p>
                                            )}
                                        </div>
                                        <div>
                                    ...
                                    <div>
                                        ...
                                        {errors.recipient_address && (
                                            <p className="text-sm text-red-600 mt-1">{errors.recipient_address}</p>
                                        )}
                                    </div>
                                </CardContent>
                            </Card>
                                ...
                                <CardContent className="space-y-4">
                                    <div>
                                        ...
                                        {errors.package_description && (
                                            <p className="text-sm text-red-600 mt-1">{errors.package_description}</p>
                                        )}
                                    </div>
                                    <div>
                                        ...
                                        {errors.package_weight && (
                                            <p className="text-sm text-red-600 mt-1">{errors.package_weight}</p>
                                        )}
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <Input
                                            ...
                                            onChange={(e) => setData('package_value', e.target.value)}
                                        />
                                    </div>
                                    <div>
                                    ...
                                    {pricing && (
                                        <div className="space-y-2">
                                            <div className="flex justify-between">
                                                <span>Base Rate:</span>
                                                <span>{formatCurrency(pricing.base_rate)}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span>Distance Rate:</span>
                                                <span>{formatCurrency(pricing.distance_rate)}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span>Weight Rate:</span>
                                                <span>{formatCurrency(pricing.weight_rate)}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span>Size Rate:</span>
                                                <span>{formatCurrency(pricing.size_rate)}</span>
                                            </div>
                                            <div className="flex justify-between font-bold text-lg pt-2 border-t">
                                                <span>Estimated Cost:</span>
                                                <span>{formatCurrency(pricing.estimated_cost)}</span>
                                            </div>
                                        </div>
                                    )}
                                </CardContent>
