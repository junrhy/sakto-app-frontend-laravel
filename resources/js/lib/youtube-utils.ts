 ...
 */
export function hideYouTubeLinks(
    content: string,
    ...
    if (showPlaceholder) {
        ...
        const placeholder =
            '<span class="text-gray-400 dark:text-gray-600 text-sm italic">[Video content available above]</span>';
        patterns.forEach((pattern) => {
    ...
    processedContent = processedContent.replace(
        /<a[^>]*href\s*=\s*["\']?https?:\/\/(www\.)?youtube\.com[^"\']*["\']?[^>]*>.*?<\/a>/gi,
        '',
    );
    processedContent = processedContent.replace(
        /<a[^>]*href\s*=\s*["\']?https?:\/\/youtu\.be[^"\']*["\']?[^>]*>.*?<\/a>/gi,
        '',
    );
